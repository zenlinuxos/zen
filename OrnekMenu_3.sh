...
function MENU_Array () {
 ...
 if [[ "${n_LenA}" = "0" ]] ; then
    ...
    if is_zero "${REPLY}" ; then
        RV="exit" ;  RETURN "${RV}">/dev/null
        return 0
    fi

    if is "${REPLY}" ">" "${n_LenA}" ] ; then
        RV="null" ;  RETURN "${RV}">/dev/null
        return 0
    fi
        if is "${REPLY}" "<=" "${n_LenA}" ; then     ### is lowerthan
        let "REPLY=REPLY-1"
        ...
        if is "${psOPTIONAL}" "=" "live" ; then       ### is_equal()
...
############ MAIN()
 clear
 echoc "INFO" "Şimdi aynı fonksiyonu  menü GeriDönüşDeğeri (GDD) - ReturnValue (RV) numeric olacak şekilde kullanalım: "
 wait 5 "NOT: Eskiden bir rakam alan menünün şimdi iki rakam veya ENTER tuşu beklediğine dikkatini çekerim :) "

  aAile=(
  "Annelerin Görevleri" \
  "Babaların Vazifeleri" \
  "Cocukların Sorumlulukları" \
  "Dedenin işleri" \
  "Ninenin işleri" \
  "Dayının işleri" \
  "Amcanın işleri" \
  ...
  "Erenlerin işleri" \
    ...
    do
         ...
         nGorev="${REPLY}"
         wait 1 "nGorev:::$nGorev ve RETURN=[$RETURN] veee RV=[$RV]"
         case "${nGorev}" in
            ...
            "3") DO_Cocuk_Gorevi
                    if isnot_okay "Menüyü Kullanmaya Devam edelim mi?" ; then break ; fi
            ;;
            "4") echoc "ARGO" "Hadi lan, ailede dede falan yok.. "
                    if isnot_okay "Menüyü Kullanmaya Devam edelim mi?" ; then break ; fi
            ;;
